...
function show_form(){
	...
	if($_SESSION['Nivel'] == 'admin'){
		...
		print("<table align='center' style='border:1; margin-top:2px' width='auto'>
			...
			<form name='form_tabla' method='post' action='$_SERVER[PHP_SELF]'>
				...
				<tr>
					<td align='center'>LOG CONTACTOS WEB</td>
				</tr>		
				<tr>
					<td>
	...
	if(!$qu){
			print("ERROR L.66 ".mysqli_error($db)."<br/>");
	} else {
...
function listfiles(){
	...
	global $rutag;
	//$rutag = "../userlog/{*}";
	$rutag = "../contactolog/{*_".$_SESSION['tablas'].".log}";
	...
	<tr><td align='center' colspan='3' class='BorderInf'>".strtoupper($_SESSION['tablas'])." ARCHIVOS LOG </td></tr>");
		...
		for($i=0; $i < $num; $i++){
			...
			//echo "** ".$arrFiles[$i]."<br>";
			$nomFiles = str_replace('../contactolog/', '', $arrFiles[$i]);
