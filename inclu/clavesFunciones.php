<?php
 ...
 function validate_form(){
	...
	global $table_name_a; 	$table_name_a = "`".$_SESSION['clave']."cliente`";
...
function show_form($errors=[]){
	...
	<form name='Perdidos' method='post' action='claves'>
			<tr>
				<th colspan=2>
			...
			<tr>
				<td colspan='2' align='center'>
					<form name='closewindow' action='$_SERVER[PHP_SELF]' onsubmit=\"window.close()\">
						<input type='submit' value='CERRAR VENTANA' class='botonNaranja' />
...
function process_form(){
	...
	//$eml = "%".$_POST['Email']."%";
	$sqlc =  "SELECT * FROM `$db_name`.$table_name_a WHERE `Email` = '$_POST[Email]' AND `dni` = '$_POST[dni]' AND `ldni` = '$_POST[ldni]' LIMIT 1 ";
	$qc = mysqli_query($db, $sqlc);
	
	if(!$qc){print("ERROR L.187: ".mysqli_error($db)."<br/><br/>");
		...
		if(mysqli_num_rows($qc)== 0){ 
			...
			print ("<table align='center' style=\"border:0px;margin_bottom:4px;margin-top:4px\">
						<tr>
							<td colspan=2 align='center'>
					<form name='modifica' action='claves' method='POST'>");
			
			while($rowc = mysqli_fetch_assoc($qc)){
										
	print ("<input name='Email' type='hidden' value='".$rowc['Email']."' />
			...
			<tr>
				<td colspan=2 align='center'>
			<img src='img/imgCliente/".$rowc['img']."' height='60px' width='45px' />
				</td>
			</tr>
			<tr>
				<td colspan=2 align='center'>
			...
			<!-- 	-->
				<tr>
					<td colspan=2 align='center'>Usuario: ".$rowc['Usuario']."</td>
				</tr>
				<tr>
					<td colspan=2 align='center'>Pass: ".$rowc['Pass']."</td>
				</tr>");
		} /* Fin del while.*/
		print("<tr>
					<td colspan=2 align='center'>
						<input type='submit' value='ENVIAR MIS DATOS VIA MAIL' class='botonVerde' />
 ...
 function process_Mail(){	 
	...
	*/
			...
			print("<table align='center' style=\"margin-top:20px;margin-bottom:20px\">
					...
					<a href='http://juanbarrospazos.blogspot.com.es/' target='_blank' class='botonVerde'>
							CONTACTO WEBMASTER
					</a>
						<form name='closewindow' action='$_SERVER[PHP_SELF]' onsubmit=\"window.close()\">
							<input type='submit' value='CERRAR VENTANA' class='botonNaranja' />
